...
async function carregarCombos() {
    ...
    document.getElementById('pessoaSelect').innerHTML =
        ...
        pessoas.map(pessoa => `
            <option value="${Number(pessoa.id)}">${AtendeLabApi.escape(pessoa.nome)}</option>
        `).join('');

    document.getElementById('tipoSelect').innerHTML =
        '<option value="">Selecione</option>' +
        tipos.map(tipo => `
            <option value="${Number(tipo.id)}">${AtendeLabApi.escape(tipo.nome)}</option>
        `).join('');
}
    ...
    try {
        ...
        tbody.innerHTML = atendimentos.map(atendimento => {
            ...
            const classeStatus =
                ...
                atendimento.status === 'em_andamento' ? 'text-bg-warning' :
                'text-bg-primary';
